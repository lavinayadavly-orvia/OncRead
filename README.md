...
## Morning automation publish path

The daily oncology-review automation should finish with a repo publish step, not only a local content refresh.

- Use `node scripts/publish-morning-briefing.mjs` after the review and browser verification pass
- The script rebuilds `data/portfolio.json` and `data/editions/`, stages only briefing-owned files, commits with the latest changelog date, and pushes the current branch
- Use `--dry-run` to inspect what would be published
- Use `--no-push` to create the local release commit without publishing

The current visible morning edition is derived from the latest dated entry in `dashboard/CHANGELOG.md`, so review-only days still become dated archive editions instead of leaving the UI stuck on an older prepared date.
# OncRead - Onco updates made easy
